def check(i):
    return i!="." and not i.isdigit()

def endRow(r2, i):
    if check(r2[i-1]) or check(r2[i]) or check(r2[i+1]):
        return True
    return False

def endColumn(r1,r2,r3,i):    
    if i == 0:
        if check(r1[i]) or check(r1[i+1]) or check(r2[i+1]) or check(r3[i]) or check(r3[i+1]):
            return True
    else:
        if check(r1[i]) or check(r1[i-1]) or check(r2[i-1]) or check(r3[i]) or check(r3[i-1]):
            return True
...
def middle(r1,r2,r3,i):
    if check(r2[i-1]) or check(r2[i+1]) or check(r1[i-1]) or check(r1[i]) or check(r1[i+1]) or check(r3[i-1]) or check(r3[i]) or check(r3[i+1]):
        return True
    return False

def topCorner(r1,r2):
    if check(r1[1]) or check(r2[0]) or check(r2[1]):
        return True
    return False

def bottomCorner(r1,r2):
    if check(r1[len(r1)-2]) or check(r2[len(r2-1)]) or check(r2[len(r2)-2]):
        return True
    return False

def sumPartNums(input):
        ...
        while countJ < len(i):
            ...
            if i[countJ].isdigit():
                    if countI == 0 and countJ == 0:
                        check = topCorner(lines[countI], lines[countI+1])
                    elif countI == 0 and countJ == len(i)-1:
                        check = topCorner(lines[countI-1], lines[countI])
                    elif countI == len(lines)-1 and countJ == 0:
                        check = bottomCorner(lines[countI],lines[countI-1])
                    elif countI == len(lines)-1 and countJ == len(i)-1:
                        check = bottomCorner(lines[countI-1], lines[countI])
                    elif countI == 0:
                        check = endRow(lines[countI+1], countJ)
                    elif countI == len(lines)-1:
                        check = endRow(lines[countI - 1], countJ)
                    elif countJ == 0:
                        check = endColumn(lines[countI-1],lines[countI], lines[countI+1], countJ)
                    elif countJ == len(i)-1:
                        check = endColumn(lines[countI-1], lines[countI], lines[countI+1], countJ)
                    else:
                        check = middle(lines[countI-1], lines[countI], lines[countI+1], countJ)
            if check:
                x = 1
                if countJ >= 2 and i[countJ-2].isdigit() and i[countJ-1].isdigit():
                    num+=i[countJ-2]
                    num+=i[countJ-1]
                elif countJ >= 1 and i[countJ-1].isdigit(): 
                    num+=i[countJ-1]
                    x+=1
                num+=i[countJ]
                if len(num)<3 and countJ <= len(i)-3 and i[countJ+1].isdigit() and i[countJ+2].isdigit():
                    num+=i[countJ+1]
                    num+=i[countJ+2]
                    x+=2
                elif len(num)<3 and countJ <= len(i)-2 and i[countJ+1].isdigit(): 
                    num+=i[countJ+1]
                    x+=1
...
input = """................................................965..583........389.................307.................512......................395.....387
